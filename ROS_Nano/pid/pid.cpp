...
PidObject::PidObject() : error_(3, 0), filtered_error_(3, 0), error_deriv_(3, 0), filtered_error_deriv_(3, 0)
{
  ...
  node_priv.param<std::string>("topic_from_controller", topic_from_controller_, "/bed_cmd_vel");
  ...
  control_effort_pub_ = node.advertise<geometry_msgs::Twist>(topic_from_controller_, 1);
...
void PidObject::doCalcs()
{
  ...
  if (new_state_or_setpt_)
  {
                         ...
                         (ros::Time::now() - last_setpoint_msg_time_).toSec() <= setpoint_timeout_))
    {
    	...
    	twist.angular.z = control_effort_;
    	twist.linear.y = 1.5;
    	control_effort_pub_.publish(twist);
